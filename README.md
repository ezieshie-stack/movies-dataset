...
This project analyzes **5,009 movies** across five decades (1970 to 2017) to answer that question with data.
...
| **Merged Master** | 5,009 | 42 columns covering financial, creative, social, and temporal features |

**Data Quality Issues Found:**
- Budget missing for 10% of films; revenue missing for 13%
- Genre data stored as raw JSON strings, requiring custom parsing
...
| **Mid ($15M to $40M)** | **1,319** | **68.5%** | **86.5%** | **Best risk-adjusted: consistent, limited downside** |
| High ($40M to $100M) | 988 | 72.9% | Moderate | Solid performers, but capital-intensive |
| Mega ($100M+) | 310 | 90.6% | Lower | Highest success rate, but when they fail, losses are catastrophic |

**Budget to Revenue correlation: 0.73.** Spending more *correlates* with higher revenue, but the ROI tells the opposite story. The marginal return on each additional dollar of budget decreases.

**Business Impact:** Blockbusters have the highest success rate (90.6%), but they require $100M+ per bet. A single failure wipes out multiple wins. Mid-budget films ($15M to $40M) offer the **most consistent returns at manageable risk**.
...
| **Action / Adventure** | Moderate | High total revenue, but requires massive budgets and carries high downside risk |
| **Drama** | Lowest | Most oversaturated genre, with too many films chasing the same audience |
...
I modeled the movie lifecycle as an **Investment-to-Profitability funnel**, using the same methodology found in sales pipeline analysis, to identify exactly where conversion drops:
...
- **Vote Count to Revenue: r = 0.78.** The strongest predictor of box office performance is audience engagement, not budget or critical ratings.
- **PG-13** films hit the broadest demographic and are the financial sweet spot for audience reach.
...
|----------------|-----------|
| **Weight portfolio toward $15M to $40M productions** | Best risk-adjusted ROI; limits catastrophic downside |
...
- Success rate would have been **84.6 to 85.7%**, compared to the industry's **72.5%** overall
...
| Industry Trends (1970 to 2017) | Correlation Matrix |
|:--:|:--:|
...
├── outputs/
│   ├── tableau/                    # Analysis-ready datasets
...
| **No inflation adjustment** | A $100M budget in 1995 is not the same as $100M in 2015; comparisons across decades are approximate |
